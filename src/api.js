export const handleApiStats = async (env, SECURITY_HEADERS) => {
			...
			try {
				// ⚡ Bolt: Chain .then(res => res.json()) directly to fetch calls so JSON parsing begins immediately as responses arrive,
				// rather than waiting for all fetch headers to resolve before starting parsing on the slowest request.
				const [
