...
<head>
	...
	<script>
		tailwind.config = { darkMode: 'class', theme: { extend: { colors: { gray: { 900: '#111827', 800: '#1f2937', 700: '#374151' } } } } }
	</script>
	<style>
		body { background-color: #0f1115; color: #e5e7eb; font-family: 'Inter', sans-serif; }
		.card { background-color: #1a1d24; border: 1px solid #2d3139; border-radius: 0.75rem; padding: 1.5rem; transition: all 0.2s; }
		.card:hover { border-color: #3b4252; box-shadow: 0 4px 20px rgba(0,0,0,0.2); }
...
<body class="min-h-screen p-6 antialiased">
	<div class="max-w-6xl mx-auto flex flex-col gap-6">
		<header class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-gray-800">
			<div>
				<h1 class="text-3xl font-bold tracking-tight text-white mb-1"><span class="gradient-text">VibeNVR</span> Telemetry</h1>
				<p class="text-sm text-gray-400">Anonymous global usage statistics for the open-source NVR.</p>
			</div>
			<div class="mt-4 sm:mt-0 flex items-center gap-2 text-xs text-gray-400 bg-gray-800/50 px-3 py-1.5 rounded-full border border-gray-700">
		...
		<!-- Error State -->
		<div id="error-msg" class="hidden bg-red-900/20 border border-red-500/50 text-red-200 p-4 rounded-lg flex justify-between items-center">
			<span>Failed to load telemetry data. Check your Cloudflare API credentials.</span>
			<button onclick="fetchStats()" class="text-sm bg-red-500/20 hover:bg-red-500/40 px-3 py-1 rounded transition">Retry</button>
		</div>
		...
		<div id="dashboard" class="hidden flex flex-col gap-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<div class="card flex flex-col justify-center">
					<h3 class="text-sm font-medium text-gray-400 mb-2 uppercase tracking-wide">Active Installations (7 days)</h3>
					<div class="text-5xl font-extrabold text-white" id="kpi-active">-</div>
				</div>
				<div class="card flex flex-col justify-center">
					<h3 class="text-sm font-medium text-gray-400 mb-2 uppercase tracking-wide">Total Installations (All Time)</h3>
					<div class="text-5xl font-extrabold text-blue-400" id="kpi-total">-</div>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<!-- Versions -->
				<div class="card">
					<h3 class="text-lg font-semibold text-white mb-4">Active Versions</h3>
					<div class="relative h-64"><canvas id="chart-versions"></canvas></div>
				</div>

				<!-- CPUs -->
				<div class="card lg:col-span-2">
					<h3 class="text-lg font-semibold text-white mb-4">Popular CPU Models</h3>
					<div class="relative h-72"><canvas id="chart-cpus"></canvas></div>
				</div>
			</div>
			
			<footer class="mt-8 text-center text-xs text-gray-500 border-t border-gray-800 pt-6">
	...
	<script>
			...
			charts[id] = new Chart(ctx, {
				...
				options: {
					...
					scales: isBar ? {
						...
						y: { beginAtZero: true, grid: { color: '#2d3139' } }
		...
		function renderDashboard(data) {
			document.getElementById('kpi-active').innerText = data.active_installs;
			document.getElementById('kpi-total').innerText = data.total_installs;

			const barPalette = ['#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#f59e0b', '#10b981', '#6b7280'];
			const piePalette = ['#60a5fa', '#c084fc', '#f472b6', '#fb7185', '#fbbf24', '#34d399', '#9ca3af'];
